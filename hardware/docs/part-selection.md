...
| 6.2 | TVS: standoff ~24 V, clamp < 40 V, rated for the feed current | **DOWNGRADED, fix round 1 Finding 6.** Candidate: Littelfuse/Bourns/ST SMBJ24A (unidirectional TVS, DO-214AA/SMB). No primary manufacturer datasheet PDF was successfully opened despite 5 attempts across two fix rounds (Bourns 403, Littelfuse 403, ST timeout, two further Farnell-mirror guesses that resolved to unrelated documents, and a datasheet4u "wrapper" page that turned out to be an Octopart-sourced parametric summary, not a real Littelfuse PDF, once actually opened and read). The figures previously reported (24 V standoff, 38.9 V clamp, 600 W/15.4-15.5 A peak pulse) are corroborated across multiple independent distributor listings but **per the rule that a PASS must rest on a primary datasheet, this is not sufficient** — no manufacturer PDF was actually read. | **UNVERIFIED** — a human must open a genuine Littelfuse/Bourns/Vishay/ST SMBJ-series PDF directly (not a distributor product page or an aggregator wrapper) and confirm VWM (standoff), VC (clamping voltage) at its stated IPP test current, and PPP, before this row can be called PASS. The previously-reported numbers are a reasonable starting point for that check, not a substitute for it. |
| 6.3 | PTC: hold >= 0.42 A **at 65 degC** (not just at 23 degC), trip below the boost's 2.5 A limit, Vmax >= 24 V, 4 required (one per RGB string) | **RE-DERATED 2026-09-12, local file actually opened this pass: `hardware/datasheets/Littelfuse_1206L_PTC_series.pdf`** (Littelfuse 1206L Series PolySwitch PPTC, "POLY-FUSE Resettable PTCs," Rev. 03/22/17, 6 pages). p.1 "Electrical Characteristics": row "1206L050/24," marking "F2": Ihold = 0.50 A, Itrip = 1.00 A, Vmax = 24 Vdc (23 degC reference) — the prior candidate. **p.2 "Temperature Rerating" table, row "1206L050/24" (Hold Current (A) by Ambient Operating Temperature): -40C 0.76, -20C 0.68, 0C 0.60, 20C 0.50, 40C 0.43, 50C 0.38, 60C 0.33, 70C 0.29, 85C 0.24.** Linear interpolation for 65 degC (spec 9.4's design-target midpoint, between the tabulated 60C and 70C columns): 0.33 + (0.29 - 0.33) x (65-60)/(70-60) = 0.33 - 0.02 = **0.31 A at 65 degC** — against the 0.42 A sustained load (spec 2.3/2.4), a **26% shortfall**, slightly worse than the previous pass's 0.25-0.30 A estimate. **Exhaustive family check (same p.1 table, every part number with Vmax >= 24 V):** 1206L005/60 (0.05 A, 60 V), 1206L010/60 (0.10 A, 60 V), 1206L012 (0.125 A, 30 V), 1206L016 (0.16 A, 30 V), 1206L020 (0.20 A, 24 V), 1206L035/30 (0.35 A, 30 V), **1206L050/24 (0.50 A, 24 V) — the highest-current 1206L device that clears 24 V.** Every 1206L device with a higher 23 degC hold current (1206L075 variants through 1206L200, 0.75-2.00 A) is rated Vmax <= 16 V (most are 6-8 V), which fails the 24 V rail outright — confirmed by reading every row of the same table, not sampled. | **FAIL — CONFIRMED, no 1206L variant satisfies this at 24 V.** The family's own derating table (p.2), read and applied in full this pass, shows 0.31 A at 65 degC vs 0.42 A required — below the load, matching the corner-cutout failure mode described previously. Because 1206L050/24 is already the highest-hold-current 1206L part number that also clears 24 V (the exhaustive check above), **no substitution within the 1206L series closes the gap** — this is a family-wide ceiling, not a mis-pick within the family. **What a human must fetch:** a larger-package PPTC datasheet not present in this repo's `hardware/datasheets/` folder — e.g. Littelfuse's **1812L series** (larger 1812 SMD body, the vendor's own next size up from 1206L) or an equivalent family from Bourns (MF-SMD) or TE Connectivity/Raychem (miniSMDC) — and check it for a device with Vmax >= 24 V whose OWN temperature-derating table (each family publishes its own; the 1206L curve above must NOT be assumed to transfer) still leaves >= 0.42 A at 65 degC, with trip current still acting below the boost's 2.5 A limit. As a rough sizing cue only, not a spec: 1206L050/24 retains 0.31/0.50 = 62% of its 23 degC rating at 65 degC, so a device in the neighborhood of "~0.7-0.8 A nominal at 23 degC" is where to start looking in a larger family — but the actual required part must be sized from that family's own published derating curve, not this one. Trip current 1.00 A vs the boost's 2.5 A ceiling and the ">1 A trip, <1 A hold" family-standard 2:1 hold:trip ratio both carry forward as design constraints for whatever replacement is found. 4x required, one per RGB string, unchanged. |
...
| TVS (front-end clamp) | Littelfuse **SMBJ24A** — **row 6.2 is UNVERIFIED, not a confirmed pass** | DO-214AA (SMB) | $0.50 (qty 1, distributor-sourced) | 32,003 (distributor-sourced) — see row 6.2: no primary manufacturer PDF was successfully read despite 5 attempts; confirm VC/VWM/PPP from a real Littelfuse/Bourns/ST datasheet before ordering |
| PTC (x4, one per RGB string) — **DO NOT ORDER. Row 6.3 FAILS: the entire 1206L family tops out at 0.50 A hold (23 degC) / ~0.31 A (65 degC) at Vmax >= 24 V — confirmed by reading every part number in the family table, not just the previous pick. No 1206L substitution closes the gap.** | ~~Littelfuse 1206L050/24WR~~ — **no replacement selected yet; needs a 1812L-class or larger PPTC family** (Littelfuse 1812L, Bourns MF-SMD, or TE Connectivity miniSMDC — datasheet not present in this repo, must be fetched) | 1206 (3216 metric), superseded | $1.88 (qty 1) / $0.841 (qty 1000) — pricing for the rejected part, not a replacement | 24,602 (Digi-Key, live page fetch) — stock for the rejected part, not a replacement |
