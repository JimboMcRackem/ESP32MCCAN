...
| 6.3 | PTC: 0.5 A hold at 24 V, trip < 1 A, 4 required | **Upgraded to a primary source, fix round 1 Finding 6.** Candidate: Littelfuse 1206L Series PolySwitch PPTC (1206 SMD), "POLY-FUSE Resettable PTCs" datasheet, Rev. 03/22/17, © Littelfuse — fetched via a Farnell-hosted direct PDF mirror this round, downloaded and parsed page-by-page with PyMuPDF (6 real pages: full electrical characteristics table for the whole 1206L family, temperature-derating table, environmental/soldering specs). "Electrical Characteristics" table, p.1: row **"1206L050/24," marking "F2": Ihold = 0.50 A, Itrip = 1.00 A, Vmax = 24 Vdc, Imax = 100 A, Pd(typ) = 0.6 W** — this is the exact base part (the "WR" suffix on 1206L050/24WR is a packaging/taping code, not a spec change). | **FAIL — REVISED 2026-09-12 (project review C4).** The 0.50 A hold figure is at the datasheet's **reference temperature (23 degC)**, and this row parsed the family's **temperature-derating table** and then never applied it. The load is **0.42 A per string** (spec 2.3), and spec 2.4 confirms daytime runs all four corners at full white **continuously**, so 0.42 A is sustained rather than a peak. At the enclosure's own design target of **58-65 degC internal** (spec 9.4), a device rated 0.50 A at 23 degC holds roughly **0.25-0.30 A** — **below the load.** Corners would cut out during sustained daytime running on a warm day and recover when cool: an intermittent fault **in the indicator path** that no bench test at room ambient reproduces. **Re-select with the derating table applied: hold >= 0.42 A AT 65 degC, i.e. a nominal >= 0.75 A at 23 degC.** Trip must still act below the boost 2.5 A limit; Vmax >= 24 V. Trip current is exactly 1.00 A rather than strictly "< 1 A" — this boundary note stands as before (PTC trip current is the guaranteed-to-trip current, not a hard sub-ceiling; this is the standard way the industry specifies it), but a human should still confirm this reading of the criterion is acceptable, or look for a lower-trip-current variant if "< 1 A" is meant literally. 4x required for the 4 PTC-per-RGB-string design context. |
...
| PTC (x4, one per RGB string) — **DO NOT ORDER. Row 6.3 now FAILS on temperature derating (C4); re-select for >= 0.42 A hold at 65 degC (>= 0.75 A nominal).** | ~~Littelfuse 1206L050/24WR~~ | 1206 (3216 metric) | $1.88 (qty 1) / $0.841 (qty 1000) | 24,602 (Digi-Key, live page fetch) |
